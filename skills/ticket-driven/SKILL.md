...
4. 着手ジャーナル 1 行（**実装チケット＝子がある場合は必ず子**。親への着手・親 In Progress は禁止）
...
**自動分割**: refine 直後に `ticket-split` 二段判定。機械的必須トリガ（DoD≥4 / 独立検証塊≥2 等）に当たれば **必ず子を作る**。分割するなら親の切り出し（プレースホルダ禁止）。1セッション完走サイズ。分割後の順は `after-split.md`。
...
| 開く条件 | 読むファイル |
|----------|----------------|
| ジャーナル文言・タイミング（`分割:` / `分割不要:` 含む） | `sections/journal.md` |
