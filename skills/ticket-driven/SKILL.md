...
**自動分割**: refine 直後に `ticket-split` 二段判定。必須トリガ A–F（DoD≥4 / 独立塊≥2 / 中心成果≥2 / 中心パス≥4 / 同居禁止 等。事前に数える指標のみ）に当たれば **必ず子を作る**。分割するなら親の切り出し（プレースホルダ禁止）。子は合格ライン（DoD 1〜3 等）。分割後の順は `after-split.md`。
