...
4. 精緻化の直後に `ticket-split` を **機能構成→セッション完走** で判定する（1セッションで収まる単一まとまりなら分割不要。粗い子は再分割）
5. UI・画面変更ならテスト計画に **Playwright 確認＋スクショ添付**を含める（添付は `redmine_helper.sh attach`。MCP不可）

### 実装中

6. 実装中は随時 `ticket-driven` のジャーナル記録テンプレートで更新
7. 開発タスクは必ずRedmineチケットで管理
8. 進捗は随時Redmineジャーナルに記録
9. 作業時間は必ずRedmineのタイムエントリーに記録する

### 完了時

10. **説明には必ずリモートリポジトリURLを記述**
11. **作業区切りごとにジャーナルにコメント**
12. **完了前は既存本文を直すだけ**（完了条件を `[x]`、リポジトリURLなど欠け埋め。薄いテンプレで説明を差し替えない）
13. **UI変更はスクショ添付済みであること**（`redmine_helper.sh attach`）
14. **欠けを埋めてから解決**
15. **作業完了後のコミットと PR はユーザー依頼時、または完了条件に含まれるとき**
